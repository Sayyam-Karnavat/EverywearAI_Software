...
class FloatingBrowser(QMainWindow):
    ...
    def init_ui(self):
        # Define storage path for browser session
        ...
        self.profile = QWebEngineProfile("my_profile", self)
        self.profile.setPersistentStoragePath(storage_path)
        self.profile.setPersistentCookiesPolicy(QWebEngineProfile.ForcePersistentCookies)

        # Optional: Set User-Agent (mimic Chrome)
        self.profile.setHttpUserAgent(
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36"
        )

        # Create a new browser page with the profile
        ...
        # Size options
        small_width, small_height = int(self.screen_width * 0.3), int(self.screen_height * 0.4)
        medium_width, medium_height = int(self.screen_width * 0.4), int(self.screen_height * 0.5)
        large_width, large_height = int(self.screen_width * 0.5), int(self.screen_height * 0.65)
        ...
        self.size_menu.addAction(self.create_menu_action("Small", "#0b6d58", self.resize_browser, small_width, small_height))
        self.size_menu.addAction(self.create_menu_action("Medium", "#0b6d58", self.resize_browser, medium_width, medium_height))
        self.size_menu.addAction(self.create_menu_action("Large", "#0b6d58", self.resize_browser, large_width, large_height))
        ...
        self.prompt_menu.addAction(self.create_simple_action("Create", "#0d846b", self.create_prompt))
        self.prompt_menu.addAction(self.create_simple_action("Open", "#0d846b", self.open_prompt))
    ...
    def closeEvent(self, event):
        """ Ensure proper cleanup to avoid warnings """
