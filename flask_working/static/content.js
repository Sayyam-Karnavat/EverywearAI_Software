  ...
  fetch("/overlay")
    ...
    .then((html) => {
      ...
      sendBtn.addEventListener("click", () => {
        ...
        const apiKey = overlayContainer.querySelector("#api-key-input").value.trim();  // Get the API key
        ...
        if (userMessage && apiKey) {
          ...
          // Make API call to Flask backend with user query and API key
          fetch("/get_response", {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
              query: userMessage,
              api_key: apiKey  // Send the API key along with the user query
            }),
          })
          ...
          alert("Please enter both a query and an API key.");
        }
      });
